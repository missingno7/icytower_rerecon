{
    ...
    y=y+fh+31;
    for (i=1; i<=max_posts && i+offset-1<numProfiles; i++,y+=fh) {
        char icon;
        int selected;
        icon=i+offset-1<1 ? '~' : '{';
        ...
        if (selected) {
            ...
            rectfill(bmp,x+7,y-47,x+263,y+fh,fg);
            solid_mode();
        }
        textprintf_ex(bmp,font,x+8,y,fg,-1,"%c %c %s %s",
                      selected ? '>' : ' ',icon,profiles[i+offset-1].handle,
                      stricmp(profiles[i+offset-1].handle,current_handle)==0 ? "(current)" : "");
    }
