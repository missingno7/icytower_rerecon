{
    ...
    /* 3246 */
    currentX = (int)ply[player_id]->x;
    dX = lastX - currentX;
    if (currentX > lastX) dX = -dX;
    /* 3247 */
    currentY = (int)ply[player_id]->y;
    dY = lastY - currentY;
    if (currentY > lastY) dY = -dY;
    /* 3248 */
    if (currentX < lastX) midX = lastX - dX / 2;
    /* 3249 */
    else midX = lastX + dX / 2;
    /* 3250 */
    if (currentY < lastY) midY = lastY - dY / 2;
    /* 3251 */
    else midY = lastY + dY / 2;

    /* 3253 */
    solid1 = is_solid(&map, currentX - 11, currentY);
    /* 3254 */
