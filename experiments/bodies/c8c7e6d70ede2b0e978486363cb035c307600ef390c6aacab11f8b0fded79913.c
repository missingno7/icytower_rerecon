{
    ...
    int th=((BITMAP *)data[66].dat)->h;
    int mh=((BITMAP *)data[65].dat)->h;
    int bh=((BITMAP *)data[64].dat)->h;
    int lh=listHeight/bh;
    int bmpHeight=lh>2 ? lh-1 : 2;
    BITMAP *bmp=create_bitmap(((BITMAP *)data[66].dat)->w,bh+th+bmpHeight*mh);
    ...
    while (!closeButtonClicked && !done) {
        ...
        if ((key[KEY_ESC] || key[KEY_ENTER] || key[KEY_SPACE]) && canDone)
            done=1;
        if (!key[KEY_ESC] && !key[KEY_ENTER] && !key[KEY_SPACE])
            canDone=1;
        while (cycle_count <= 0)
            rest(2);
