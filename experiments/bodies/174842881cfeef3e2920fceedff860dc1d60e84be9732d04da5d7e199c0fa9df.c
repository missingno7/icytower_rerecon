{
    ...
    if (recording) {
        ...
        if (ply[player_id]->dead) {
            ...
            Trecord *current=&demo->data[rec_pos];
            unsigned char flags=control->flags&0x93;
            if (current->key_flags&0x80) {
                current[1].key_flags=0x80;
                current[1].cycle_count=0;
                current[2].key_flags=0;
                current[2].cycle_count=0;
            }
            else {
                if (current->key_flags==flags)
                    current->cycle_count++;
                else {
                    rec_pos++;
                    current=&demo->data[rec_pos];
                    current->key_flags=flags;
