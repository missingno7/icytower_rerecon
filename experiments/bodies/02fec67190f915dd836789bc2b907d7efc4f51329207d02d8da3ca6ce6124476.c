{
    ...
    /* 467 */
    double view_percentage = (double)max_posts / num_itr_files;
    double view_offset;
    ...
    view_offset = (1.0 - view_percentage) * 260.0f /
                  (num_itr_files - max_posts) * offset;
    rectfill(bmp, x + 297,
             y + 32 + view_offset,
             x + 303,
    ...
    if (max_posts > 0 && offset < num_itr_files) {
        int row = y + 31 + fh;
        curr_filename = 0;
        show_directory = 0;
        selected_version = 0;
        for (i = 1; i <= max_posts && offset + i - 1 < num_itr_files; i++, row += fh) {
        char name[1024];
        ...
        int file_index = offset + i - 1;
        Treplay_post *post = &file_list[file_index];
        char marker;
