{
    ...
    if (!blit_mode) {                                          /* 2282 */
    ...
    else if (blit_mode == 5 || blit_mode == 6) {
        double dx, dy;
        int x, y;
        if (blit_mode == 5) {
            dx = ply[player_id]->x - 160.0;
            dy = ply[player_id]->y - 160.0;
            if (dx <= 0.0) x = 0;
            ...
            dx = ply[player_id]->x - 80.0;
            dy = ply[player_id]->y - 80.0;
            if (dx <= 0.0) x = 0;
        ...
        stretch_blit(bmp, screen, x, y, blit_mode == 5 ? 320 : 160, blit_mode == 5 ? 240 : 120, 0, 0, 640, 480);
    }    release_screen();
}