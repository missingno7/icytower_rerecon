{
    ...
    if (recording) {
        ...
        if (ply[player_id]->dead) {
            demo->data[rp+1].key_flags=0x80;
            demo->data[rp+1].cycle_count=0;
            demo->data[rp+2].key_flags=0;
            demo->data[rp+2].cycle_count=0;
        }
        else {
            if (demo->data[rp].key_flags&0x80) {
                demo->data[rp+1].key_flags=0x80;
                demo->data[rp+1].cycle_count=0;
                demo->data[rp+2].key_flags=0;
                demo->data[rp+2].cycle_count=0;
            }
            else if ((flags=control->flags&0x93, demo->data[rp].key_flags==flags))
                demo->data[rp].cycle_count++;
            else {
                rp++;
                rec_pos=rp;
                demo->data[rp].key_flags=flags;
                demo->data[rp].cycle_count=0;
            }
        }
    }
    else {
