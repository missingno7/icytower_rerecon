    ...
    while (!closeButtonClicked) {
        ...
        switch (menu_return) {
            ...
            case 103: {
                int sub_ret;
                mp->pos = 0;
                sub_ret = handle_menu((Tmenu *)data, mp, ctrl, bmp, callback, x, y, dx);
            ...
            case 109: {
                ...
                sld->value = MIN(sld->max, sld->value + sld->step);
                break;
            }
            case 110: {
                Tmenu_slider *sld = (Tmenu_slider *)data;
                sld->value = MAX(sld->min, sld->value - sld->step);
                break;
            }
            case 112: {
                Tmenu_selection *sel = (Tmenu_selection *)data;
                sel->value = MIN(sel->size - 1, sel->value + 1);
                break;
            }
            case 119:
            case 121: {
                Tmenu_char_selection *sel = (Tmenu_char_selection *)data;
                sel->value = MIN(sel->max, sel->value + 1);
                break;
            }
            case 111:
            case 118:
            case 120: {
                Tmenu_char_selection *sel = (Tmenu_char_selection *)data;
                sel->value = sel->value > 1 ? sel->value - 1 : 0;
                break;
            }
            case 113: {
                int *box = (int *)data;
