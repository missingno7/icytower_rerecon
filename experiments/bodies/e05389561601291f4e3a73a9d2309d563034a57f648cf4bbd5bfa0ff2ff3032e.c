{
    ...
    sum = (((((((((((((r->biggest_lost_combo + 1) * 17 +
        r->no_combo_top_floor * 127) + r->floor_shrink * 102) +
        r->floor_size * 17) + 3702) + r->start_speed * 163) +
        r->speed_increase * 23) + r->gravity * 88) +
        r->random_seed * 329) + r->tc_posts * 127) +
        r->rejump * 13) + r->score * 17 + 17) +
        (r->combo + 1) * 649) + (r->floor + 1) * 113;
    for (i = 0; i < 5; i++)
