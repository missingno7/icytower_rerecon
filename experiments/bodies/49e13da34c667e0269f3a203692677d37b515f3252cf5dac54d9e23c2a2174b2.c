{
    ...
    current_profile_handle = current_profile->handle;
    old_font = font;
    font = data[54].dat;
    ...
    while (!closeButtonClicked && !done) {
        ...
        ctrl_wait = is_any(ctrl) ?
                    (ctrl_wait > 0 ? ctrl_wait - 1 : ctrl_wait) : 0;
        ...
        if (keypressed()) {
            ...
            case 77: {
                if (stricmp(profiles[profileIndex].handle, "guest") &&
                    stricmp(profiles[profileIndex].handle, current_profile_handle)) {
                    char buff[256];
