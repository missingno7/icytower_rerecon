{
    ...
    int scrollHeight;
    int i;
    Tavailable_profile *row_profiles;
    int row_y;
    char *current_handle;
    ...
    row_profiles=profiles+offset-1;
    row_y=y+fh+31;
    for (i=1;
         i<=max_posts && i+offset-1<numProfiles;
         i++,row_y+=fh) {
        char icon;
        int selected;
        icon=i+offset-1<1 ? '~' : '{';
        if (i+offset-1==selection) {
            drawing_mode(5,0,0,0);
        ...
        textprintf_ex(bmp,font,x+8,row_y,fg,-1,"%c %c %s %s",
                      selected ? '>' : ' ',icon,row_profiles[i].handle,
                      stricmp(row_profiles[i].handle,current_handle)==0 ? "(current)" : "");
    }
