{
    ...
    poll_control(&menu_params.ctrl, 0);                                 /* 494 */
    while (is_any(&ctrl) || is_any(&menu_params.ctrl) || key[KEY_ESC]) { /* 495 */
        poll_control(&ctrl, 0); poll_control(&menu_params.ctrl, 0); rest(2);
    }
    clear_keybuf();                                            /* 500 */
    done = 0;
    status = 0;
    while (!closeButtonClicked && !done) {                     /* 502 */
        cycle_count = 0;                                       /* 503 */
        poll_control(&ctrl, 0); poll_control(&menu_params.ctrl, 0);     /* 504 */
        if (is_left(&ctrl) || is_left(&menu_params.ctrl)) status = -1;  /* 507 */
        if (is_right(&ctrl) || is_right(&menu_params.ctrl)) status = 0; /* 510 */
        if (key[KEY_ESC]) {                                    /* 514 */
            done = -1;
            status = 0;
        }
        if (is_fire(&ctrl) || is_fire(&menu_params.ctrl) || is_enter(&menu_params.ctrl)) done = -1; /* 519 */
        if (choice) {                                          /* 523 */
            vsync();                                           /* 525 */
    ...
    poll_control(&menu_params.ctrl, 0);                                 /* 536 */
    while (is_any(&ctrl) || is_any(&menu_params.ctrl) || key[KEY_ESC] || key[KEY_ENTER]) { /* 537 */
        poll_control(&ctrl, 0); poll_control(&menu_params.ctrl, 0); rest(2); /* 538 */
    }
    clear_keybuf();                                            /* 543 */
