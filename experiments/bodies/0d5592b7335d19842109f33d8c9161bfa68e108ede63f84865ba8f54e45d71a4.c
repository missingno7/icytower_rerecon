{
    ...
    /* 467 */
    int total_files = num_itr_files;
    double view_percentage = (double)max_posts / total_files;
    double view_offset;
    ...
    /* 479 */
    rect(bmp, x + 5, y + 30, x + w - 5, y + h - 5, fg);
    /* 479 */
    rect(bmp, x + w - 15, y + 30, x + w - 5, y + h - 5, fg);
    view_offset = (1.0 - view_percentage) * 260.0f /
                  (total_files - max_posts) * offset;
    rectfill(bmp, x + 297,
             y + 32 + view_offset,
             x + 303,
             y + 32 + view_offset + view_percentage * 260.0f,
             fg);
    /* 487 */
    solid_mode();
    ...
    isCustom = rep ? is_custom_replay(rep) : 0;
    set_clip_rect(bmp, x + 6, 0,
                  x + 390 - (isCustom ? text_length(font, "CUSTOM GAME  ") : 0),
                  bmp->h - 1);
    if (curr_filename)
        textprintf_ex(bmp, font, x + 10, y + 315, mg, -1, "%s", curr_filename);
    set_clip_rect(bmp, 0, 0, bmp->w - 1, bmp->h - 1);
    ...
    if (rep) {
        ...
        if (isCustom)
            textprintf_right_ex(bmp, font, x + 390 - text_length(font, "CUSTOM GAME  "),
