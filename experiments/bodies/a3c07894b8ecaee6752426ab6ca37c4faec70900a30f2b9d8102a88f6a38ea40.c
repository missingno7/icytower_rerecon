{
    ...
    if (!closeButtonClicked) do {   /* 3536 (3534 tests playing; both tests and both loop
            ...
            if (!recording)
                ...
                if (is_playing_custom_game) {
                    ...
                    if (gotHigh) {                                                       /* 4749 */
                        ...
                        if (isGuest)
                            summary_text = "You're playing in guest mode. Start a profile and "
                                           "record your progress!";
                        else
                            summary_text = hints[new_rand() % 45];                        /* 4775 */
                    } else {
                        if (isGuest)
                            summary_text = "You're playing in guest mode. Start a profile and "
                                           "record your progress!";
                        else
                            summary_text = hints[new_rand() % 45];                        /* 4770 */
                    }
                    strcpy(summary_scroller_message, summary_text);
                }
            }
