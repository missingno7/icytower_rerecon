{
    ...
    if (!blit_mode) {                                          /* 2282 */
        ...
        int x, y;
        double delta;
        delta = ply[player_id]->x - 160.0;`n        if (delta > 320.0) x = 320;`n        else if (delta <= 0.0) x = 0;`n        else x = (int)delta;
        delta = ply[player_id]->y - 160.0;`n        if (delta > 240.0) y = 240;`n        else if (delta <= 0.0) y = 0;`n        else y = (int)delta;
        stretch_blit(bmp, screen, x, y, 320, 240, 0, 0, 640, 480); /* 2307 */
    }
    else if (blit_mode == 6) {                                 /* 2309 */
        int x, y;
        double delta;
        delta = ply[player_id]->x - 80.0;`n        if (delta > 520.0) x = 520;`n        else if (delta <= 0.0) x = 0;`n        else x = (int)delta;
        delta = ply[player_id]->y - 80.0;`n        if (delta > 360.0) y = 360;`n        else if (delta <= 0.0) y = 0;`n        else y = (int)delta;
