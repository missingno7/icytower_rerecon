{
    ...
    y+=32;
    for (i=1;
         i<=max_posts && offset-1+i<numProfiles;
         i++,y+=fh) {
        char icon;
        int selected;
        icon=offset-1+i<1 ? '~' : '{';
        if (offset-1+i==selection) {
            drawing_mode(5,0,0,0);
        ...
        textprintf_ex(bmp,font,x+8,y,fg,-1,"%c %c %s %s",
                      selected ? '>' : ' ',icon,
                      profiles[offset-1+i].handle,
                      stricmp(profiles[offset-1+i].handle,current_profile->handle)==0 ? "(current)" : "");
    }
