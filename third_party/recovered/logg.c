/* Allegro 4.4.1 logg, with the historical memory extension reconstructed.
 * Upstream copyright (c) 2007 Trent Gamblin.
 * License: ../licenses/logg-license.txt. */
#include <string.h>
#include "allegro.h"
...
struct logg_memfile_context {
    void *pData;
    size_t iDataSize;
    size_t iCursor;
};
size_t logg_vf_memfile_read(void *ptr, size_t size, size_t nmemb, void *context)
{
    struct logg_memfile_context *pCtx=context;
    size_t iRead=size*nmemb;
    if (pCtx->iCursor+iRead > pCtx->iDataSize) iRead=pCtx->iDataSize-pCtx->iCursor;
    if (iRead) {
        memcpy(ptr, (char *)pCtx->pData+pCtx->iCursor, iRead);
        pCtx->iCursor+=iRead;
    }
    return iRead;
}
int logg_vf_memfile_seek(void *context, ogg_int64_t offset, int whence)
{
    struct logg_memfile_context *pCtx=context;
    if (whence==SEEK_SET) pCtx->iCursor=offset;
    else if (whence==SEEK_CUR) pCtx->iCursor+=offset;
    else if (whence==SEEK_END) pCtx->iCursor=pCtx->iDataSize;
    return 0;
}
long logg_vf_memfile_tell(void *context)
{
    struct logg_memfile_context *pCtx=context;
    return pCtx->iCursor;
}
int logg_vf_memfile_close(void *context)
{
    struct logg_memfile_context *pCtx=context;
    free(pCtx);
    return 0;
}
SAMPLE* logg_load_internal(OggVorbis_File *pVorbisFile)
{
    vorbis_info *vi;
    SAMPLE *samp;
    int numRead;
	...
	samp->freq = vi->rate;
	samp->priority = 128;
	samp->len = ov_pcm_total(pVorbisFile, -1);
	samp->loop_start = 0;
	samp->loop_end = samp->len;
	samp->data = _al_malloc(sizeof(unsigned short) * samp->len * 2);

	while ((numRead = ov_read(pVorbisFile, buf, logg_bufsize,
...
SAMPLE *logg_load_memory(void *pData, size_t iSize)
{
    ...
    struct logg_memfile_context *pCtx=malloc(sizeof(struct logg_memfile_context));
    pCtx->pData=pData;
    pCtx->iDataSize=iSize;
    pCtx->iCursor=0;
    callbacks.read_func=logg_vf_memfile_read;
    callbacks.seek_func=logg_vf_memfile_seek;
    callbacks.tell_func=logg_vf_memfile_tell;
    callbacks.close_func=logg_vf_memfile_close;
    if (ov_open_callbacks(pCtx,&ovf,0,0,callbacks)!=0) {
        strncpy(allegro_error,"ov_open_callbacks failed.",ALLEGRO_ERROR_SIZE);
        free(pCtx);
        return 0;
    }
    return logg_load_internal(&ovf);
}
